...
```bash
uv add "openroboto-protocol==0.4.0"
# or
pip install "openroboto-protocol==0.4.0"
```
...
```bash
uv add "openroboto-protocol[schemas]==0.4.0"   # backend only
```
...
Consumers pin an exact version (`openroboto-protocol==0.4.0`). Floating versions are
